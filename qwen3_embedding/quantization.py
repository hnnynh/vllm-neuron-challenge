...
"""
Qwen3Embedding quantization abstraction.
...
class QuantizationSpec:
    ...
    A ``None``-valued ``Qwen3EmbeddingConfig.quant_spec`` means "not quantized". When
    ...
    def from_hf_quantization_config(
        ...
        raise ValueError(
            ...
            "Qwen3Embedding currently supports: 'modelopt', 'compressed-tensors'."
        )
...
def _parse_modelopt(quantization_config: dict[str, Any]) -> QuantizationSpec:
    ...
    if quant_algo != "FP8":
        raise ValueError(
            "Qwen3Embedding currently only supports ModelOpt quant_algo='FP8', "
            f"got quant_algo={quant_algo!r}."
        )
    if kv_cache_algo != "FP8":
        raise ValueError(
            "Qwen3Embedding currently requires ModelOpt kv_cache_quant_algo='FP8', "
    ...
    if not any(p == "lm_head" or fnmatch("lm_head", p) for p in patterns):
        raise ValueError(
            "ModelOpt FP8 checkpoints for Qwen3Embedding must list 'lm_head' in "
...
def resolve_attention_mlp_classes(
    ...
    Args:
        quant_spec: Result of :meth:`QuantizationSpec.from_hf_quantization_config`.
            ...
            from :mod:`vllm_neuron.model.qwen3_embedding.model` are returned and
    ...
    if quant_spec is None:
        return _bf16.Qwen3EmbeddingAttention, _bf16.Qwen3EmbeddingMLP
    ...
    if scheme is QuantScheme.FP8_STATIC_PER_TENSOR:
        ...
        return _fp8.Qwen3EmbeddingAttention, _fp8.Qwen3EmbeddingMLP
